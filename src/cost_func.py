...
def distance_cost(route: List[List[int]]) -> float:
    '''
    ...
    '''
    full_route = [l.tolist() for l in route]

    for i, courier_route in enumerate(full_route):
        if np.sum(np.array(DEMAND)[courier_route]) > CAPACITY[i]:
            return np.inf
    ...
    total_costs = np.multiply(total_distance, np.array(SALARY)).sum() 
